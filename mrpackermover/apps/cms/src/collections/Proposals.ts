...
const PACK_OPTIONS = ['Standard Wrap', 'Bubble Wrap', 'Wooden Crate', 'Original Box', 'Blanket Wrap'].map(
  (v) => ({ label: v, value: v }),
);
...
export const Proposals: CollectionConfig = {
  ...
  fields: [
    ...
    { name: 'quoteNo', type: 'text', label: 'Quote no.', admin: { position: 'sidebar', readOnly: true } },
    {
      name: 'lead',
      type: 'relationship',
      relationTo: 'leads',
    ...
    { name: 'clientName', type: 'text', label: 'Customer', admin: { position: 'sidebar', readOnly: true } },
              ...
              fields: [
                {
                  ...
                  fields: [
                    ...
                    { name: 'froms', type: 'text', label: 'Origin state / area', admin: { width: '50%' } },
                  ],
                },
                {
                  type: 'row',
                  fields: [
                    { name: 'to', type: 'text', label: 'Destination city', admin: { width: '50%' } },
                    { name: 'tos', type: 'text', label: 'Destination state / area', admin: { width: '50%' } },
                  ],
                },
                {
                  type: 'row',
                  fields: [
                    { name: 'date', type: 'date', label: 'Move date', admin: { width: '33%', date: { pickerAppearance: 'dayOnly' } } },
                    { name: 'house', type: 'select', label: 'Truck size', defaultValue: '14 ft', options: TRUCK_OPTIONS, admin: { width: '33%' } },
                    { name: 'dist', type: 'text', label: 'Distance', admin: { width: '34%', placeholder: '≈ 1,180 km' } },
                  ],
                },
                { name: 'svc', type: 'select', label: 'Service type', defaultValue: 'Domestic Household', options: SERVICE_OPTIONS },
              ],
            },
          ],
        },
              ...
              fields: [
                {
                  ...
                  fields: [
                    ...
                    { name: 'qty', type: 'number', label: 'Qty', defaultValue: 1, admin: { width: '15%' } },
                    { name: 'pack', type: 'select', label: 'Packing', defaultValue: 'Standard Wrap', options: PACK_OPTIONS, admin: { width: '40%' } },
                  ],
                },
              ...
              fields: [
                {
                  type: 'row',
                  fields: [
                    { name: 'name', type: 'text', label: 'Charge description', admin: { width: '70%' } },
              ...
              fields: [
                {
                  type: 'row',
                  fields: [
                    { name: 'gstRate', type: 'number', label: 'GST rate (%)', defaultValue: 18, admin: { width: '25%' } },
                    { name: 'goodsValue', type: 'number', label: 'Declared goods value (₹)', admin: { width: '25%' } },
                    { name: 'insRate', type: 'number', label: 'Insurance rate (%)', defaultValue: 0.3, admin: { width: '25%', description: '0 to drop insurance' } },
                    { name: 'validDays', type: 'number', label: 'Validity (days)', defaultValue: 15, admin: { width: '25%' } },
                  ],
                },
                { name: 'pay', type: 'text', label: 'Payment terms', defaultValue: '25% advance · balance before unloading' },
              ],
            },
          ],
        },
              ...
              fields: [
                {
                  type: 'row',
                  fields: [
                    { name: 'name', type: 'text', defaultValue: 'MrPackerMover', admin: { width: '50%' } },
                    { name: 'tag', type: 'text', label: 'Tagline', defaultValue: 'Trusted Household & Office Relocation', admin: { width: '50%' } },
                  ],
                },
                {
                  type: 'row',
                  fields: [
                    { name: 'phone', type: 'text', label: 'Phone / WhatsApp', defaultValue: '+91 99104 26834', admin: { width: '50%' } },
                    { name: 'mobile', type: 'text', label: 'Alternate mobile', admin: { width: '50%' } },
                  ],
                },
                {
                  type: 'row',
                ...
                { name: 'addr', type: 'text', label: 'Registered address' },
                {
                  ...
                  fields: [
                    ...
                    { name: 'rep', type: 'text', label: 'Consultant / Prepared by', admin: { width: '50%' } },
                  ],
                },
              ],
            },
