...
// @ts-nocheck
/*!
 * proposal-pdf.ts — 2-page A4 PDF generator, originally ported verbatim from the Proposal
 * Studio (proposal-studio/app.js). Framework-agnostic: `genProposalPDF(d)` takes a data
 * object and returns a PDF Blob.
 *
 * The layout, spacing and typography are still the approved Proposal Studio design and
 * should not be rearranged. Two things were deliberately changed, on request:
 *
 *   Colour   The document was navy with crimson accents, a palette that shared nothing
 *            with the brand. It now uses the website's own tokens - royal blue with
 *            orange. See the CO block for why the accent roles are named by job rather
 *            than by hue.
 *
 *   Logo     The masthead drew a tile with a generic truck glyph, which was never the
 *            company's mark. It now embeds the real lockup as a PDF image XObject
 *            (see logo-data.ts, generated by scripts/build-pdf-logo.mjs).
 *
 * Other changes vs the editor script: the auto-init at the end is replaced by a returned
 * generator, and `quoteNo` is read from `d.quoteNo` instead of a module global. The
 * editor's DOM helpers are left in place but unused.
 */
import { PDF_LOGO } from './logo-data';
import { shortProposalService } from '../../lib/proposal-service.js';
...
const _proposalPdf = (function () {
  ...
  /* Kept in step with DEFAULT_SERVICES in collections/Proposals.ts - this copy is the
     fallback used when a caller passes no services at all.

     The GPS line is gone: it promised live tracking on every quote, and /track is still
     a stub, so it was a commitment nobody could honour. Packing says 3-layer because
     that is what the crews actually wrap. */
  var defaultServices=[
    'Professional packing, Premium 3-layer materials, room-wise labelling',
  ...
  function render(d){
    ...
    var s1='<section class="sheet s1"><header class="masthead"><div class="mast-top">'+
      ...
      '<div class="p-block"><div class="p-sec"><h3>Inventory of Articles</h3><div class="rule"></div><div class="meta">'+items.length+' items · '+totalArticles+' articles</div></div>'+
  ...
  /* Place the embedded logo. `cm` sets a transform whose scale IS the display size in
     points, and PDF's origin is bottom-left, so the y given here (top-left, like every
     other primitive in this file) is flipped once. q/Q brackets it so the transform
     does not leak into whatever is drawn next. */
  function image(O,name,x,y,w,h){O.push('q '+ff(w)+' 0 0 '+ff(h)+' '+ff(x)+' '+ff(PH-(y+h))+' cm /'+name+' Do Q');}
  ...
  /* Document palette, taken from the website's design tokens (ui-tokens/theme.css).
     This replaces the navy-and-crimson scheme inherited from the Proposal Studio,
     which shared no colour with the brand.

     The names describe the ROLE, not the hue, because the two accent roles have
     opposite contrast requirements and swapping them silently breaks legibility:

       deep      navy-900  masthead and headline values
       brand     brand-600 eyebrow labels on white AND fills sitting under white text.
                 12.4:1 against white either way, which orange-500 cannot manage - at
                 3.5:1 it fails small text in both directions, so it is kept off text.
       flash     orange-500  rules and bars only, where nothing sits on top
       onDeep    orange-300  accent text ON the deep masthead (6.8:1 there)
       ember     orange-700  small accent text on light rows (7.7:1 on white) */
  var CO={deep:'0f0063',deeper:'0a0042',brand:'1d00b2',brandMid:'2a0dbe',onDeep:'ff945e',flash:'ff5500',ember:'b03700',tintBrand:'efecff',ink:'1a1c2e',body:'464a60',soft:'6f7389',line:'e5e6ef',tint:'f7f6ff',tint2:'efecff',white:'ffffff',onDeepSoft:'a99cec',onBrandSoft:'c9c0ff',deepRule:'3b1fb0'};
  ...
  /* Display size of the embedded lockup, in points. The artwork is 600px wide, so at
     150pt it renders at 4x - crisp when zoomed or printed. Height follows the
     artwork's own ratio rather than a fixed number, so replacing the logo cannot
     stretch it. */
  var LOGO_W=150,LOGO_H=150*(PDF_LOGO.height/PDF_LOGO.width);

  /* The generator is handed whatever the proposal form holds, and an editor can empty
     a repeater. Reading .length off an absent list threw and produced no PDF at all,
     which is a worse failure than a section coming out short - so every list is read
     through here. */
  ...
  function sheet1(d){
    ...
    var validTill=prettyDate(addDays(isoToday(),d.validDays));
    fillRect(O,0,0,PW,112,CO.deep);
    fillRect(O,0,112,PW,4,CO.flash);
    /* The lockup carries the wordmark and the strapline as artwork, so the typeset
       company name and tagline that used to sit beside a badge are gone - rendering
       the brand name in Helvetica next to the real logo looked like a fallback. A
       company that sets its own name in the CMS still gets it, under the mark. */
    image(O,'Im0',CM,22,LOGO_W,LOGO_H);
    if(c.name&&c.name!=='MrMoverPacker')txt(O,CM,22+LOGO_H+13,c.name,9.5,1,CO.onDeepSoft);
    txt(O,RX,30,'MOVING PROPOSAL',9,1,CO.onDeep,'r');
    txt(O,RX,50,d.quoteNo,15,1,CO.white,'r');
    txt(O,RX,68,'Date: '+prettyDate(isoToday()),8.5,0,CO.onDeepSoft,'r');
    txt(O,RX,80,'Valid till: '+validTill,8.5,0,CO.onDeepSoft,'r');
    seg(O,CM,94,RX,94,CO.deepRule,0.9);
    ...
    contactRow(O,mc,CM,106,CO.brand,CO.white,CO.onBrandSoft,links,CW);
    ...
    /* "Noida, Uttar Pradesh, India" does not fit one 15pt line in 175pt, and it used
       to be cut to "Noida, Uttar Pradesh, I..". Split at the first comma instead: the
       place name keeps the large type, and the rest of the address wraps underneath at
       9pt, joined with any sub-line the caller passed. A single-word location renders
       exactly as it did before. Two wrapped lines is the cap - beyond that an address
       is being pasted in, not written, and the card should not grow without limit. */
    var RW=175;
    function routeText(v,sub){
      var t=san(v||'-'),i=t.indexOf(','),head=i>0?t.slice(0,i).trim():t,rest=i>0?t.slice(i+1).trim():'';
      var tail=[rest,san(sub||'')].filter(Boolean).join(' - ');
      return {head:trunc(head,15,1,RW),tail:tail?wrap(tail,9,0,RW).slice(0,2):[]};
    }
    var rFrom=routeText(mv.from,mv.froms),rTo=routeText(mv.to,mv.tos);
    /* The card grows only as far as the taller side needs, so a short route keeps the
       original 54pt height and the page below it does not shift. */
    var rmH=54+Math.max(0,Math.max(rFrom.tail.length,rTo.tail.length)-1)*11;
    rrect(O,CM,y,CW,rmH,10,CO.line,true,1);
    txt(O,CM+18,y+17,'FROM',8,1,CO.brand);txt(O,CM+18,y+35,rFrom.head,15,1,CO.deep);
    for(var ri=0;ri<rFrom.tail.length;ri++)txt(O,CM+18,y+47+ri*11,rFrom.tail[ri],9,0,CO.soft);
    txt(O,RX-18,y+17,'TO',8,1,CO.brand,'r');txt(O,RX-18,y+35,rTo.head,15,1,CO.deep,'r');
    for(ri=0;ri<rTo.tail.length;ri++)txt(O,RX-18,y+47+ri*11,rTo.tail[ri],9,0,CO.soft,'r');
    var cy=y+28,mx1=CM+CW/2-46,mx2=CM+CW/2+46;
    seg(O,mx1,cy,mx2,cy,CO.onDeep,1.3,true);
    fillRect(O,mx1-2,cy-2,4,4,CO.brand);fillRect(O,mx2-2,cy-2,4,4,CO.brand);
    icirc(O,CM+CW/2,cy,12,CO.brand,0,true);icon(O,'truck',CM+CW/2-8.5,cy-8.5,17,CO.white,1.3);
    var cyps=y+rmH+10,chH=30;
    rrect(O,CM,cyps,CW,chH,8,CO.tint);
    var chips=[['MOVE DATE',prettyDate(mv.date)],['TRUCK SIZE',mv.house],['DISTANCE',mv.dist||'-'],['SERVICE',shortProposalService(mv.svc)]],chIco=['cal','truck','ruler','home'],cwid=CW/4;
    for(var ci=0;ci<4;ci++){var cx=CM+ci*cwid;if(ci>0)seg(O,cx,cyps+6,cx,cyps+chH-6,CO.line,0.8);badge(O,cx+17,cyps+15,8.5,CO.tintBrand,chIco[ci],CO.brand,11);txt(O,cx+33,cyps+13,chips[ci][0],7.5,1,CO.soft);txt(O,cx+33,cyps+24,trunc(String(chips[ci][1]||''),9.5,1,cwid-44),9.5,1,CO.ink);}
    y=cyps+chH+18;
    ...
    rrect(O,CM,y,CW,gh,10,CO.deep);
    /* Orange, not brand blue. This bar marks the single most important number on the
       document, and #1d00b2 on the #0f0063 card is barely a shade apart - the accent
       may as well not have been drawn. Nothing sits on top of it, so the full-strength
       orange is safe here (see the CO block on why orange stays off text). */
    fillRect(O,CM,y+12,4,gh-24,CO.flash);
    txt(O,CM+22,y+25,'GRAND TOTAL',11,1,CO.onDeep);
    ...
    rrect(O,CM,y,CW,26,8,CO.tintBrand);
    badge(O,CM+16,y+13,7,CO.brand,'clock',CO.white,9);
    txt(O,CM+30,y+16,'This estimate is valid till '+validTill+'.   Book early to lock your move date & pricing.',9.5,0,CO.deep);
    /* Page 2's footer is pinned to the sheet (fy = PH-58); page 1's simply followed the
       content, so on a short quote it stopped halfway down and looked like the page had
       been cut off. Pinned to the same bottom margin as the body text.

       Math.max keeps the old behaviour as the floor: if a long inventory runs past the
       pinned position, the footer still falls after the content rather than being
       overprinted by it. */
    y=Math.max(y+26+14,PH-44);
    seg(O,CM,y,RX,y,CO.line,0.8);
    txt(O,CM,y+12,c.name+'   -   '+d.quoteNo,8,1,CO.brand);
  ...
  function sheet2(d){
    ...
    fillRect(O,0,0,PW,52,CO.deep);fillRect(O,0,52,PW,4,CO.flash);
    /* No company name beside it: the lockup already says the name, and at 93pt wide
       the old text position (CM+44) now falls inside the artwork. */
    image(O,'Im0',CM,13,LOGO_W*0.62,LOGO_H*0.62);
    txt(O,RX,24,'TERMS & AGREEMENT',9,1,CO.onDeep,'r');
    txt(O,RX,38,d.quoteNo,9,0,CO.onBrandSoft,'r');

    var y=86,half=(CW-24)/2;
    secHead(O,"What's Included",'',y);y+=18;
    var svcs=list(d.services),colH=Math.ceil(svcs.length/2),yA=y,yB=y;
    for(var i=0;i<svcs.length;i++){
      ...
      badge(O,cx+8,cyy-1,8,CO.brand,'check',CO.white,10);
      txt(O,cx+24,cyy+1,trunc(head,11,1,half-28),11,1,CO.deep);
    ...
    var tms=list(d.terms),tcolH=Math.ceil(tms.length/2),tHalf=(CW-24)/2,tyA=y,tyB=y;
    for(var k=0;k<tms.length;k++){
      var tL=k<tcolH,tx2=tL?CM:CM+tHalf+24,tyy=tL?tyA:tyB;
      rrect(O,tx2,tyy-8,15,15,3.5,CO.brand);
    ...
    txt(O,CM,y+14,'For '+c.name,11,1,CO.deep);txt(O,CM,y+26,'Authorised Signatory - Date',9,0,CO.soft);
    txt(O,CM+sw+40,y+14,cu.name||'Customer',11,1,CO.deep);txt(O,CM+sw+40,y+26,'Accepted - Signature & Date',9,0,CO.soft);
    y+=44;rrect(O,CM,y,CW,28,8,CO.tintBrand);
    badge(O,CM+16,y+14,7,CO.brand,'check',CO.white,9);
    txt(O,CM+30,y+17,'Kindly sign and return a copy of this proposal to confirm your booking. This estimate supersedes any prior verbal quote.',9,2,CO.deep);

    var fy=PH-58;
    /* Matches the rules under both mastheads, and for the same reason: a brand-blue
       rule on the near-black footer band was invisible. */
    fillRect(O,0,fy,PW,58,CO.deeper);fillRect(O,0,fy,PW,3,CO.flash);
    ...
    contactRow(O,fc,CM,fy+42,CO.brand,CO.white,CO.onBrandSoft,links,CW);
    txt(O,RX,fy+24,'Page 2 of 2 - Terms',8,0,CO.onDeepSoft,'r');
    return {content:O.join('\n'),links:links};
  }

  function assemble(pages){
    var N=pages.length,firstFont=3+N*2,nF1=firstFont,nF2=firstFont+1,nF3=firstFont+2;
    /* One image XObject, shared by both pages. Allocated after the fonts so the link
       annotations keep numbering on from it. */
    var nImg=nF3+1,body=[],obj=nImg;
    ...
    for(i=0;i<N;i++){var pnum=3+i*2,cnum=4+i*2,st=pages[i].content,links=pages[i].links||[],annots=[];
      for(var j=0;j<links.length;j++){var lk=links[j];obj++;
        ...
        annots.push(obj+' 0 R');}
      body[pnum]='<< /Type /Page /Parent 2 0 R /Resources << /Font << /F1 '+nF1+' 0 R /F2 '+nF2+' 0 R /F3 '+nF3+' 0 R >> /XObject << /Im0 '+nImg+' 0 R >> >>'+(annots.length?' /Annots ['+annots.join(' ')+']':'')+' /Contents '+cnum+' 0 R >>';
    ...
    /* The logo, as raw JPEG bytes. DCTDecode means the PDF stores the compressed
       stream untouched, so there is no re-encoding and no quality loss. atob turns the
       base64 into a binary string; the writer below takes the low byte of each code
       unit, which is exactly those bytes back again. */
    var jpg=atob(PDF_LOGO.base64);
    body[nImg]='<< /Type /XObject /Subtype /Image /Width '+PDF_LOGO.width+' /Height '+PDF_LOGO.height+' /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length '+jpg.length+' >>'+String.fromCharCode(10)+'stream'+String.fromCharCode(10)+jpg+String.fromCharCode(10)+'endstream';
    var out='%PDF-1.4\n%âãÏÓ\n',off=[],total=obj;
