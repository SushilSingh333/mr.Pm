...
const SERVICE_SUMMARY = {
  ...
  'office-shifting': 'Office and IT-asset relocation planned around your no-entry and lift windows.',
...
const CITIES = [
  {
    slug: 'delhi', name: 'Delhi', jobs: 56, onTime: 96, settle: 11,
    editorial: [
  ...
  {
    slug: 'gurgaon', name: 'Gurgaon', jobs: 41, onTime: 95, settle: 12,
    editorial: [
  ...
  {
    slug: 'noida', name: 'Noida', jobs: 33, onTime: 96, settle: 10,
    editorial: [
  ...
  {
    slug: 'mumbai', name: 'Mumbai', jobs: 62, onTime: 93, settle: 12,
    editorial: [
  ...
  {
    slug: 'bengaluru', name: 'Bengaluru', jobs: 48, onTime: 95, settle: 11,
    editorial: [
  ...
  {
    slug: 'pune', name: 'Pune', jobs: 37, onTime: 96, settle: 10,
    editorial: [
  ...
  {
    slug: 'hyderabad', name: 'Hyderabad', jobs: 39, onTime: 95, settle: 11,
    editorial: [
  ...
  {
    slug: 'chennai', name: 'Chennai', jobs: 34, onTime: 94, settle: 12,
    editorial: [
...
const GLOBAL_FAQS = [
  { question: 'Will the price change on delivery day?', answer: 'No. The written quote is fixed; the only things that can change it are stated up front on the quote.' },
  { question: 'Are my goods insured in transit?', answer: 'Yes, transit insurance is available and the premium is shown as a line item on your quote — never hidden.' },
  { question: 'How do you verify the crew and vehicle?', answer: 'Every crew and vehicle is ID-verified; you can check who is coming and in what vehicle before they load.' },
  { question: 'What happens if something is damaged?', answer: 'Damage is rare, but when it happens we settle. We publish our median settlement time on the claims page.' },
];

// ── Helpers ──────────────────────────────────────────────────────────────────
const SHARD = {
  home: 'sitemaps/core.xml', 'service-hub': 'sitemaps/core.xml',
  'city-hub': 'sitemaps/cities.xml', 'city-service': 'sitemaps/city-services.xml',
  locality: 'sitemaps/localities.xml', route: 'sitemaps/routes.xml',
};
const lslug = (name) => name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
const abs = (p) => (p === '/' ? `${ORIGIN}/` : `${ORIGIN}${p}`);
const priceFrom = Math.min(...BANDS.map((b) => b.base));
const reviewsFor = (city) => [
  { jobRef: `${city.slug.slice(0, 3).toUpperCase()}-2026-1001`, author: 'Rahul S.', rating: 5, text: `On time in ${city.name}, nothing broken, and the final bill matched the quote to the rupee.`, date: '2026-07-20' },
  { jobRef: `${city.slug.slice(0, 3).toUpperCase()}-2026-1002`, author: 'Meena K.', rating: 5, text: `The ${city.name} crew wrapped every fragile item and reassembled the beds. Would use again.`, date: '2026-07-12' },
  { jobRef: `${city.slug.slice(0, 3).toUpperCase()}-2026-1005`, author: 'Vikram T.', rating: 4, text: `40 minutes late in ${city.name} but they called ahead and adjusted the bill. Fixed it.`, date: '2026-07-04', response: 'Thanks for the fair rating and for letting us make it right.' },
];
const cityFaqs = (city) => [
  { question: `What is included in a ${city.name} home shift?`, answer: `Packing, loading, transport, unloading, and basic reassembly. Society gate passes in ${city.name} are arranged by your coordinator.` },
  { question: `Do you handle service-lift booking in ${city.name}?`, answer: `Yes. Your ${city.name} coordinator confirms the lift slot and gate timings before the crew is dispatched.` },
  { question: `How early should the crew reach in ${city.name}?`, answer: `Early, because several ${city.name} societies restrict vehicle entry until mid-morning. We plan the load-out around it.` },
];

const pages = [];
const row = (r) => {
  pages.push({
    pageType: r.pageType, path: r.path, canonical: abs(r.path), slug: r.slug,
    sitemapShard: SHARD[r.pageType], cacheTag: r.cacheTag, lastmod: LASTMOD,
    title: r.title, metaDescription: r.metaDescription, h1: r.h1, noindex: false,
    breadcrumbs: r.breadcrumbs ?? [], relatedLinks: r.relatedLinks ?? [], inboundLinks: [],
    data: r.data ?? {},
  });
};

const cyclic = (arr, i, n) => Array.from({ length: n }, (_, k) => arr[(i + k + 1) % arr.length]).filter((x) => x !== arr[i]);
...
for (const [i, s] of SERVICES.entries()) {
  ...
  const cityLinks = hasCityService
    ? CITIES.map((c) => ({ path: `/packers-and-movers/${c.slug}/${s.slug}`, anchor: `${s.name} in ${c.name}`, group: 'cities' }))
    : CITIES.slice(0, 6).map((c) => ({ path: `/packers-and-movers/${c.slug}`, anchor: `Serving ${c.name}`, group: 'cities' }));
  row({
    pageType: 'service-hub', path: `/services/${s.slug}`, slug: s.slug, cacheTag: 'service-hub',
    title: `${s.name} Services in India – MrPackerMover`.slice(0, 60), h1: `${s.name} Services`,
    breadcrumbs: [{ path: '/', anchor: 'Home' }],
    relatedLinks: [...cityLinks, ...siblings.map((x) => ({ path: `/services/${x.slug}`, anchor: x.name, group: 'services' }))],
    data: { serviceName: s.name, summary: SERVICE_SUMMARY[s.slug], inclusions: ['Packing & materials', 'Loading & unloading', 'Transport', 'Basic reassembly'], exclusions: ['Floor rise above 2 floors without a lift', 'Long carry beyond 50 m', 'Extra packing for oversized items'] },
  });
}
...
for (const [ci, city] of CITIES.entries()) {
  ...
  const cityRoutes = ROUTES.filter((r) => r.o === city.slug || r.d === city.slug).map((r) => {
    const oc = CITIES.find((c) => c.slug === r.o), dc = CITIES.find((c) => c.slug === r.d);
    return { path: `/routes/${r.o}-to-${r.d}`, anchor: `${oc.name} to ${dc.name}`, group: 'routes' };
  });
  row({
    pageType: 'city-hub', path: cityPath, slug: city.slug, cacheTag: `city-${city.slug}`,
    title: `Packers and Movers in ${city.name} – MrPackerMover`.slice(0, 60), h1: `Packers and Movers in ${city.name}`,
    breadcrumbs: [{ path: '/', anchor: 'Home' }],
    relatedLinks: [
      ...SERVICES.slice(0, 6).map((s) => ({ path: CITY_SERVICE_SLUGS.includes(s.slug) ? `${cityPath}/${s.slug}` : `/services/${s.slug}`, anchor: `${s.name} in ${city.name}`, group: 'services' })),
      ...localitySlugs.map((l) => ({ path: `${cityPath}/${l.slug}`, anchor: l.name, group: 'localities' })),
      ...cityRoutes,
    ],
    data: {
      cityName: city.name, editorial: city.editorial, rateBands: BANDS,
      stats: { jobs12m: city.jobs, onTimePct: city.onTime, avgSettlementDays: city.settle, damagePct: 1 },
      reviews: reviewsFor(city), faqs: cityFaqs(city), priceFrom,
  ...
  for (const [li, l] of localitySlugs.entries()) {
    row({
      pageType: 'locality', path: `${cityPath}/${l.slug}`, slug: `${city.slug}/${l.slug}`, cacheTag: `city-${city.slug}`,
      title: `Packers and Movers in ${l.name}, ${city.name} – MrPackerMover`.slice(0, 60),
      h1: `Packers and Movers in ${l.name}, ${city.name}`,
      breadcrumbs: [{ path: '/', anchor: 'Home' }, { path: cityPath, anchor: city.name }],
      relatedLinks: cyclic(localitySlugs, li, 3).map((x) => ({ path: `${cityPath}/${x.slug}`, anchor: `Movers in ${x.name}`, group: 'siblings' })),
      data: {
        localityName: l.name, cityName: city.name, cityPath, editorial: [
        ...
        ], rateBands: BANDS, stats: { jobs12m: Math.round(city.jobs / 3), onTimePct: city.onTime, avgSettlementDays: city.settle },
        reviews: reviewsFor(city).slice(0, 2), faqs: cityFaqs(city).slice(0, 3), priceFrom,
      },
    });
  }
  ...
  for (const s of SERVICES.slice(0, 2)) {
    row({
      pageType: 'city-service', path: `${cityPath}/${s.slug}`, slug: `${city.slug}/${s.slug}`, cacheTag: `city-${city.slug}`,
      ...
      relatedLinks: [
        ...
        ...cyclic(CITIES, ci, 4).map((c) => ({ path: `/packers-and-movers/${c.slug}/${s.slug}`, anchor: `${s.name} in ${c.name}`, group: 'nearby' })),
      ],
      data: {
        cityName: city.name, serviceName: s.name, editorial: city.editorial.slice(0, 1),
        rateBands: BANDS, stats: { jobs12m: Math.round(city.jobs * 0.7), onTimePct: city.onTime, avgSettlementDays: city.settle },
        reviews: reviewsFor(city).slice(0, 2), faqs: cityFaqs(city).slice(0, 3),
        inclusions: ['Packing & materials', 'Loading & unloading', 'Transport', 'Basic reassembly'],
        exclusions: ['Floor rise above 2 floors without a lift', 'Long carry beyond 50 m'], priceFrom,
      },
    });
  }
}
...
for (const r of ROUTES) {
  const oc = CITIES.find((c) => c.slug === r.o), dc = CITIES.find((c) => c.slug === r.d);
  row({
    pageType: 'route', path: `/routes/${r.o}-to-${r.d}`, slug: `${r.o}-to-${r.d}`, cacheTag: 'route',
    title: `${oc.name} to ${dc.name} Packers and Movers – MrPackerMover`.slice(0, 60),
    h1: `${oc.name} to ${dc.name} Packers and Movers`,
    breadcrumbs: [{ path: '/', anchor: 'Home' }, { path: `/packers-and-movers/${r.o}`, anchor: oc.name }],
    relatedLinks: [
      ...
      ...(ROUTES.some((x) => x.o === r.d && x.d === r.o) ? [{ path: `/routes/${r.d}-to-${r.o}`, anchor: `${dc.name} to ${oc.name}`, group: 'reverse' }] : []),
    ],
    data: {
      originName: oc.name, destName: dc.name, roadKm: r.km, transitDays: r.days, frequency: '3× a week',
      overnight: [`Load by 8 PM at the ${oc.name} end, roll out after the no-entry window lifts, and cover the trunk route overnight. You take no leave — the truck is already moving while you sleep.`],
      borderNotes: 'Inter-state permits handled end to end. Monsoon can add a day on the ghat sections.',
      rateBands: BANDS, priceFrom,
    },
  });
}
...
// Home
row({
  pageType: 'home', path: '/', slug: 'home', cacheTag: 'home',
  ...
  relatedLinks: [
    ...
    ...CITIES.map((c) => ({ path: `/packers-and-movers/${c.slug}`, anchor: c.name, group: 'cities' })),
    ...ROUTES.map((r) => ({ path: `/routes/${r.o}-to-${r.d}`, anchor: `${CITIES.find((c) => c.slug === r.o).name} to ${CITIES.find((c) => c.slug === r.d).name}`, group: 'routes' })),
  ],
  data: {
...
for (const p of pages) {
  for (const link of [...p.relatedLinks, ...p.breadcrumbs]) {
    ...
    if (target && target.path !== p.path) target.inboundLinks.push({ path: p.path, anchor: link.anchor });
  }
}
...
const manifest = {
  ...
  siteOrigin: ORIGIN,
  org: {
    brandName: 'MrPackerMover', legalName: 'AAJneeti Connect Private Limited',
    gstin: '07AAAAA0000A1Z5', cin: 'U63030DL2020PTC000000', yearsOperating: 6,
    insurancePartner: 'ICICI Lombard', registeredOffice: 'Registered office address — confirm before launch.',
    complaintSla: 'First response within 24 hours.', phone: '+91-00000-00000', whatsapp: '+91-00000-00000',
