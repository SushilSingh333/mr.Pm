 ...
 * returns a PDF Blob. DO NOT restyle — this preserves the exact approved PDF design. The only
 * changes vs the editor script: the auto-init at the end is replaced by a returned generator,
 * and `quoteNo` is read from `d.quoteNo` (passed in) instead of a module global. The editor's
 * DOM helpers are left in place but unused.
 */
const _proposalPdf = (function () {
  'use strict';
  ...
  function sheet1(d){
    ...
    var chips=[['MOVE DATE',prettyDate(mv.date)],['TRUCK SIZE',mv.house],['DISTANCE',mv.dist||'-'],['SERVICE',mv.svc]],chIco=['cal','truck','ruler','home'],cwid=CW/4;
    ...
    var intro='Dear '+(cu.name||'Customer')+', thank you for choosing '+c.name+' for your relocation from '+mv.from+' to '+mv.to+'. Please find your itemised inventory and a fully transparent estimate below - with no hidden charges.';
    ...
    txt(O,CM,y+12,c.name+'   -   '+d.quoteNo,8,1,CO.crim);
  ...
  function sheet2(d){
    ...
    txt(O,RX,38,d.quoteNo,9,0,'c9d3e2','r');
  ...
  return { genProposalPDF: function (d) { return genPDF(d); } };
})();

export const genProposalPDF = _proposalPdf.genProposalPDF;
