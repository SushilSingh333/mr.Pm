/**
 * The in-repo blog posts. They render when the CMS `posts` collection is empty, so a
 * fresh install never shows a blank blog, and they are the source the CMS import script
 * reads when moving these articles into the database (see apps/cms import-blog).
 *
 * Kept in @mpm/shared rather than the web app so both the site and the CMS can read the
 * same copy without one reaching into the other's source tree.
 */
